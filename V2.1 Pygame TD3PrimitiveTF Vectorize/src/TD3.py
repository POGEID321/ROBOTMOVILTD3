...
class Agent:
    ...
    def choose_action(self, observation):
        ...
        if self.time_step < self.warmup:
            ...
            mu_prime = self.choose_action_graph(state)
    ...
    @tf.function
    def choose_action_graph(self, observation):
        mu = self.actor(observation)[0]
        mu_prime = mu + tf.random.normal(shape=mu.shape, stddev=self.noise)
        mu_prime = tf.clip_by_value(mu_prime, self.min_action, self.max_action)
        return mu_prime

...
class EnviromentGym(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        if options == "Random":
            ...
            while np.linalg.norm([gx - px, gy - py]) <= 600 or np.linalg.norm([gx - px, gy - py]) >= 800:
    ...
    def step(self, action):
        ...
        # delta_action = np.linalg.norm(action - self.action_prev)
        # - Calcula Recompensa 
        Mov = (0.1*action[0]**2)
        self.action_prev = action
        reward = 0.05 * Mov + 10 * new_observation_space[9]
        # reward -= 0.004 * self.interaction
        # reward -= 0.004 * delta_action

        done = False

        actual_distance = np.linalg.norm([xgoal - xplayer, ygoal - yplayer])
        if abs(actual_distance) < 0.3:
            reward += 2
            done = True
